...
function buildWheel(tier, targetContainerId, optionsArray, onSliceClick, activeValue = null) {
    ...
    optionsArray.forEach((optionText, i) => {
        ...
        // MATHEMATISCHE FIXIERUNG FÜR TEXT-LABELS:
        // Wir holen uns das echte Zentrum des umschließenden Kreises via CSS-BoundingBox!
        const rect = wrapper.getBoundingClientRect();
        const wrapperCenterW = rect.width / 2;
        const wrapperCenterH = rect.height / 2;
        
        const isMobile = window.innerWidth <= 768;
        let labelRadius = 0;
        
        if (tier === 1) {
            labelRadius = isMobile ? (rect.width * 0.43) : 250;
        } else {
            labelRadius = isMobile ? (rect.width * 0.40) : 110;
        }
        
        const posX = wrapperCenterW + labelRadius * Math.cos(midAngle);
        const posY = wrapperCenterH + labelRadius * Math.sin(midAngle);
...
// Listener um Textverschiebungen beim Drehen des Bildschirms abzufangen
