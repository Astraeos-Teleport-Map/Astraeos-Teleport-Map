// 1. Map Initialization (Geändert auf "let" für flexible Wiederverwendung)
let map = L.map('map', {
    crs: L.CRS.Simple,
    minZoom: -2,
    maxZoom: 2,
...
function buildWheel(tier, targetContainerId, optionsArray, onSliceClick, activeValue = null) {
    ...
    optionsArray.forEach((optionText, i) => {
        ...
        // Touch- und Klicksupport für Mobilgeräte kombiniert
...
// KORREKTUR: Das fehlerhafte "const map" wurde hier entfernt, damit die globale Variable genutzt wird!
...
initUiState();
loadMapMarkers();// 1. Map Initialization
const map = L.map('map', {
    crs: L.CRS.Simple,
    minZoom: -2,
    maxZoom: 2,
...
function buildWheel(tier, targetContainerId, optionsArray, onSliceClick, activeValue = null) {
    ...
    // REPARATUR: "labels-tier-TierX" Fehler behoben -> Sucht jetzt exakt nach kleingeschriebenem Klassen-String
    const oldLabels = wheelContainer.parentElement.querySelector(`.labels-tier-${tier}`);
    if (oldLabels) oldLabels.remove();
    ...
    optionsArray.forEach((optionText, i) => {
        ...
        // REPARATUR: Klick-Handler für PC und Mobile (Touch-Support hinzugefügt)
...
// Rechnet die internen Leaflet-Pixel zurück in die Anzeige (0-100)
function convertCoordinates(rawY, rawX) {
    let calculatedLon = rawX / 10;
    let calculatedLat = 100 - (rawY / 10);
...
// Updates text inside the coordinate panel
function updateCoordsDisplay(lat, lon, isStatic = false) {
    if (isStatic) {
        coordsPanel.innerHTML = `Selected:<br>Lat: <b>${parseFloat(lat).toFixed(1)}</b> | Lon: <b>${parseFloat(lon).toFixed(1)}</b>`;
        coordsPanel.style.borderLeftColor = '#ffbc00'; 
...
// Resets panel view on mouse out
function refreshCoordsToActiveState() {
    if (activeWaypointId) {
        const match = teleportWaypoints.find(p => p.id === activeWaypointId);
        if (match) {
